...
curl -L https://download.cirros-cloud.net/0.3.5/cirros-0.3.5-x86_64-disk.img | openstack image create --container-format bare --disk-format qcow2 --public 'cirros-0.3.5'
curl -L https://download.cirros-cloud.net/0.4.0/cirros-0.4.0-x86_64-disk.img | openstack image create --container-format bare --disk-format qcow2 --public 'cirros-0.4.0'
curl -L https://download.cirros-cloud.net/0.5.2/cirros-0.5.2-x86_64-disk.img | openstack image create --container-format bare --disk-format qcow2 --public 'cirros-0.5.2'
openstack network create --share --external --provider-network-type flat --provider-physical-network extnet public
openstack subnet create --subnet-range '10.10.10.0/24' --no-dhcp --gateway '10.10.10.1' --network public --allocation-pool start=10.10.10.100,end=10.10.10.200 --dns-nameserver 8.8.8.8 public-subnet
...
openstack server create --flavor m1.mini --image 'cirros-0.5.2' --network hr-network-1 --security-group default --security-group allow-ssh i1
